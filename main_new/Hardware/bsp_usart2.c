...
__IO uint32_t g_Uart2_TransferState = UART2_TRANSFER_COMPLETE;

// 定义USART2回调函数指针
UART_RxCpltCallbackFunc huart2_rxcplt_cb = NULL;
...
*********************************************************************************************************
*/
static void USART2_RxCpltHandler(UART_HandleTypeDef *huart);
...
void bsp_InitUsart2(uint32_t baudrate)
{
  ...
  */
  ...
  if (HAL_UART_Init(&huart2) != HAL_OK)
  {
	...
	// 绑定USART2专属回调函数指针
	huart2_rxcplt_cb = USART2_RxCpltHandler;
...
void Usart2_SendString(uint8_t *str)
{
  ...
  do 
  {
		USART2->TDR = (uint8_t)*(str + k);/* 将要发送的字符 ch 写入到DR寄存器 */
...
{
#if UART2_RX_DMA
	if (g_Uart2_TransferState != UART2_TRANSFER_COMPLETE)
		return;

	g_Uart2_TransferState = UART2_TRANSFER_WAIT;	/* DMA发送状态 */
...
{
	/* 1. 检查发送完成标志（TCIF1：Stream1传输完成标志） */
	if((DMA2->LISR & DMA_LIFCR_CHTIF1) != 0)
	...
	if((DMA2->LISR & DMA_LISR_TCIF1) != 0)
	{
		...
		/* 发送完成后的操作（可选，如置位发送完成标志） */
		g_Uart2_TransferState = UART2_TRANSFER_COMPLETE;
	}
    
	/* 2. 检查发送错误标志（TEIF1：Stream1传输错误标志） */
	if((DMA2->LISR & DMA_LISR_TEIF1) != 0)
	{
		...
		/* 记录发送错误 */
		g_Uart2_TransferState = UART2_TRANSFER_ERROR;
	}
    
	/* 3. 其他错误标志处理 */
	if((DMA2->LISR & (DMA_LISR_DMEIF1 | DMA_LISR_FEIF1)) != 0)
	{
		DMA2->LIFCR = DMA_LIFCR_CDMEIF1 | DMA_LIFCR_CFEIF1;
		g_Uart2_TransferState = UART2_TRANSFER_ERROR;
	}
}
#endif
