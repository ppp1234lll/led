...
#define LAN8720_RST 		   	PAout(6) 			// LAN8720复位引脚
...
u8 LAN8720_Get_Speed(void)
{
	...
	if(speed&1<<1) return 100;
	else           return 10;
}
//得到8720的双工模式
u8 LAN8720_Get_Duplex(void)
{
	u8 speed;
	speed=((ETH_ReadPHYRegister(0x00,31)&0x1C)>>2); //从LAN8720的31号寄存器中读取网络速度和双工模式
	if(speed&1<<3) return 2; // 全双工
	else           return 1; // 半双工
}
...
//	printf("DMARxDscrTab: %d..\n",ETH_RXBUFNB*sizeof(ETH_DMADESCTypeDef));
//	printf("DMATxDscrTab: %d..\n",ETH_TXBUFNB*sizeof(ETH_DMADESCTypeDef));
//	printf("Rx_Buff: %d..\n",ETH_RX_BUF_SIZE*ETH_RXBUFNB);
//	printf("Tx_Buff: %d..\n",ETH_TX_BUF_SIZE*ETH_TXBUFNB);
