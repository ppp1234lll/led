...
#define   PULSE_TIME_LOW    200
#define   PULSE_TIME_HIGH   300


extern data_collection_t sg_datacollec_t;
extern uint8_t seng_state1;
...
/* USER CODE BEGIN 1 */
extern void single_time_run(void);
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  static uint16_t time_high[12] = {0};
	...
	if(htim->Instance == TIM1)
	{
		...
		for(i = 0; i < 12; i++)
		{
			if(sg_datacollec_t.voltage[i] == 1)
			{
				time_high[i]++;
			}
			if(sg_datacollec_t.voltage[i] == 0 )
			{
				if(time_high[i] > PULSE_TIME_HIGH)
				{
					time_high[i] = 0;
				}
				
				if(time_high[i] > PULSE_TIME_LOW && time_high[i] < PULSE_TIME_HIGH)
				{
