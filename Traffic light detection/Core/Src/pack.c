#include <stdio.h>
#include <stdint.h>
#include <string.h>
#include "main.h"

// ==================== CRC8 计算函数（多项式 0x31，初始值 0x00）====================
uint8_t crc8_calculate(const uint8_t *data, uint32_t len) {
    uint8_t crc = 0x00;
    for (uint32_t i = 0; i < len; i++) {
        crc ^= data[i];
        for (uint8_t j = 0; j < 8; j++) {
            if (crc & 0x80) {
                crc = (crc << 1) ^ 0x31;  // 多项式 0x31
            } else {
                crc <<= 1;
            }
        }
    }
    return crc;
}


static const unsigned char crc8Table[256] =
{
    
};


/*******************************************************************************
* Function Name  :  calc_crc8
* Description    :  用于CRC8校验
* Input          :  *data 需要校验的首地址  len  数据的长度
* Output         :  
* Note           :  // 按照多项式 X^8+X^2+X^1+1 生成。
* Return         :  crc8 最后校验的结果
*******************************************************************************/

unsigned char calc_crc8(const unsigned char *data, const unsigned int data_len)
{
	unsigned int len = 0;
    unsigned char crc8 = 0;
	
	len = data_len;
    while (len --)
    {
        crc8 = crc8 ^ (*data++);
        crc8 = crc8Table[crc8];
    }
    return crc8;
}

// ==================== 结构体定义 ====================
//extern data_collection_t sg_datacollec_t;

// ==================== 组包函数 ====================
// 输出：返回总包长
// 输入：cmd(命令), data(结构体数据), output_buffer(输出缓冲区)
uint32_t pack_data(uint8_t cmd, const data_collection_t *data, uint8_t *output_buffer) {
    uint32_t offset = 0;

    // 1. 包头 (2字节: 0xF0 0xF0)
    output_buffer[offset++] = 0xF0;
    output_buffer[offset++] = 0xF0;

    // 2. 指令 (1字节)
    output_buffer[offset++] = cmd;

    // 3. 数据长度 (1字节，注意：长度不能超过255)
    uint32_t data_len = sizeof(data_collection_t);  // 108 字节
    if (data_len > 255) {
        printf("Error: Data length %u exceeds 1-byte limit!\n", data_len);
        return 0;
    }
    output_buffer[offset++] = (uint8_t)data_len;

    // 4. 数据 (108字节)
    memcpy(output_buffer + offset, data, data_len);
    offset += data_len;

    // 5. 计算 CRC8 校验（校验范围：指令 + 长度 + 数据）
    // 起始位置：指令字节在buffer[2]，长度为 1 + 1 + 108 = 110 字节
    uint8_t crc = calc_crc8(output_buffer + 2, 1 + 1 + data_len);

    // 6. 添加 CRC8 校验 (1字节)
    output_buffer[offset++] = crc;

    // 7. 包尾 (1字节: 0xFF)
    output_buffer[offset++] = 0xFF;
		output_buffer[offset++] = 0xFF;

    return offset;  // 返回总包长
}

