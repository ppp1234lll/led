...
#define UART2x_TX_DMA_STREAM               DMA2_Stream1
#define UART2x_RX_DMA_STREAM               DMA2_Stream0

#define UART2x_TX_DMA_REQUEST              DMA_REQUEST_USART2_TX
#define UART2x_RX_DMA_REQUEST              DMA_REQUEST_USART2_RX

#define UART2x_DMA_TX_IRQn                 DMA2_Stream1_IRQn
#define UART2x_DMA_RX_IRQn                 DMA2_Stream0_IRQn

#define UART2x_DMA_TX_IRQHandler           DMA2_Stream1_IRQHandler
#define UART2x_DMA_RX_IRQHandler           DMA2_Stream0_IRQHandler

#define UART2x_IRQn								USART2_IRQn
#define UART2x_IRQHandler					USART2_IRQHandler

enum {
	UART2_TRANSFER_WAIT,
	UART2_TRANSFER_COMPLETE,
	UART2_TRANSFER_ERROR
};
...
__IO uint32_t g_Uart2_TransferState = UART2_TRANSFER_WAIT;
...
__attribute__((section(".RAM_SRAMD2"), aligned(32))) uint8_t g_U2RxBuffer[U2_RX_SIZE];
__attribute__((section(".RAM_SRAMD2"), aligned(32))) uint8_t g_U2TxBuffer[U2_TX_SIZE];
...
void bsp_InitUsart2(uint32_t baudrate)
{
  ...
  */
	...
	*/
	...
	/*##-3- 配置DMA ##################################################*/
	/* 配置DMA发送 */
	hdma_usart2_rx.Instance = UART2x_RX_DMA_STREAM;
	hdma_usart2_rx.Init.Request = UART2x_RX_DMA_REQUEST;
	...
	hdma_usart2_rx.Init.FIFOMode = DMA_FIFOMODE_DISABLE;
	 ...
	 /* 复位DMA */
	if(HAL_DMA_DeInit(&hdma_usart2_rx) != HAL_OK)
	{
		Error_Handler(__FILE__, __LINE__);     
	}
	...
	/* USART2_TX Init */
	hdma_usart2_tx.Instance = UART2x_TX_DMA_STREAM;
	hdma_usart2_tx.Init.Request = UART2x_TX_DMA_REQUEST;
	...
	hdma_usart2_tx.Init.FIFOMode = DMA_FIFOMODE_DISABLE;
	 ...
	 /* 复位DMA */
	if(HAL_DMA_DeInit(&hdma_usart2_tx) != HAL_OK)
	{
		Error_Handler(__FILE__, __LINE__);     
	}
  ...
  // 启动 DMA 接收
	...
	/*##-4- 配置中断 #########################################*/
	/* 配置DMA发送中断 */
	HAL_NVIC_SetPriority(UART2x_DMA_TX_IRQn, 5, 0);
	HAL_NVIC_EnableIRQ(UART2x_DMA_TX_IRQn);
	
	/* 配置DMA接收中断 */
	HAL_NVIC_SetPriority(UART2x_DMA_RX_IRQn, 5, 0);
	HAL_NVIC_EnableIRQ(UART2x_DMA_RX_IRQn);
	
	/* 配置SPI中断 */
	HAL_NVIC_SetPriority(UART2x_IRQn, 5, 0);
	HAL_NVIC_EnableIRQ(UART2x_IRQn);
